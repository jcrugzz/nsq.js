...
function Reader(opts) {
  ...
  //
  // Necessary for certain environments where host is broadcast incorrectly
  // i.e. kubernetes
  //
  this.addrModify = typeof options.addrModify === 'function'
    ? options.addrModify : addrModifyDefault;
...
Reader.prototype.connect = function(){
  ...
  this.lookup(function(_, nodes){
    if (nodes) {
      self.connectToEach(nodes.map(address).map(this.addrModify));
    }
  });
...
Reader.prototype.poll = function(ms){
  ...
  this.timer = setInterval(function(){
    self.lookup(function(errors, nodes){
      ...
      var addrs = nodes.map(address).map(this.addrModify);
